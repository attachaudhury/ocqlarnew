...
$.SvgCanvas = function (container, config) {
  ...
  var attacustomaddSvgElementFromJson = function (dataurl) {
    ...
    svgCanvas.setHref(newImage, dataurl);
    svgCanvas.selectOnly([newImage])
    svgCanvas.alignSelectedElements("m", "page")
    svgCanvas.alignSelectedElements("c", "page")
  };

  attaCustomApiObject.attacustomaddSvgElementFromJson = attacustomaddSvgElementFromJson;




  var attaApplyAnimation = function (data) {
    console.log(data)
  };

  attaCustomApiObject.attaApplyAnimation = attaApplyAnimation;
  ...
  (function () {
    ...
    attatippywidgetopened = false;
    var attashowFloatingToolbar = function (config) {
      if (config.type == "text") {
        attatippywidgetopened = true;
        tippyresultlistener = config.callback;
        var textValues = {
          ...config.currentValues
        };
        var fontFamily = textValues.fontFamily || '';
        var fontSize = textValues.fontSize || '';
        var bold = textValues.bold || false;
        var italic = textValues.italic || false;
        var color = textValues.color || "#ffffff";
        var fontFamilyText = ['Times New Roman', 'Arial,Hevetica', 'Arial', 'Serif', 'Sans-serif', 'Cursive', 'Fantasy', 'Alegreya', 'AmaticSc', 'Fjalla', 'JosefinSans', 'JosefinSlub', 'Lato', 'Merriweather', 'Montserrat', 'Noto', 'Playfair', 'Roboto', 'Monospace', 'Viljamis', 'Cooperblack', 'Herrvon', 'Homemade', 'Ibmplex', 'Josefin', 'Justme', 'Karma', 'Jotione', 'Kirang', 'Kotta', 'Play', 'Pompiere'].map(el => {
          var selectedfontFamilyText = (fontFamily == el) ? 'selected' : ''
          return '<option style="padding: 2px 10px;font-family:' + el + '" value="' + el + '" ' + selectedfontFamilyText + '>' + el + '</option>'
        })
        var fontSizeText = '<input  style="height:5px;margin-bottom: 0px;border: 1px solid gainsboro;padding:10px;" type="number" onkeyup="tippyresultlistener({fontSize:this.value})" type="number" value="' + fontSize + '">'
        var boldText = (bold) ? '<input onchange="tippyresultlistener({bold:this.checked})" type="checkbox" checked>' : '<input onchange="tippyresultlistener({bold:this.checked})"  type="checkbox">';
        var italicText = (italic) ? '<input onchange="tippyresultlistener({italic:this.checked})"  type="checkbox" checked>' : '<input onchange="tippyresultlistener({italic:this.checked})"   type="checkbox">';
        var colorText = '<input value="' + color + '" onchange="tippyresultlistener({color:this.value})" style="margin-left:-3px;width:100%;height:20px;border:1px solid gainsboro;background:white" type="color">';


        attatippyinstance = tippy(event.target, {
          allowHTML: true,
          interactive: true,
          arrow: false,
          appendTo: document.body,
          content: `<div>
               <div style="background:white;border-bottom: 1px solid gainsboro;padding: 2px 5px;"><p style="margin:0px;font-weight:bold;color: #8c8b8b;">Text Setting</p></div>
               <div style="padding: 2px 1px;">
   <select class='myselect' size=3 style="width:100%;margin-bottom: 0px;border: 1px solid gainsboro;font-size:14px;font-weight:bold;" onchange="tippyresultlistener({fontFamily:this.value})">
      ${fontFamilyText}
   </select>
   <br>
   ${fontSizeText}
   <br>
   <div style="background:white;padding: 2px 8px;">
   <div style="display:inline-flex;margin-left:-5px;">
      ${boldText}
   <label style="vertical-align:text-bottom;margin-bottom:unset;"><strong>Bold</strong></label>
   </div>
   <div style="display:inline-flex;float:right">
   ${italicText}
   <label  style="vertical-align:text-bottom;margin-bottom:unset;"><strong>Italic</strong></label>
   </div>
   ${colorText}
   </div>
   </div>
        ...
        attatippyinstance.show();
      }

    }
    attahandleTippyCommand = function (data) {
      var objectKeys = Object.keys(data);
      if (objectKeys.includes('fontSize')) {
        attaCustomApiObject.setFontSize(data.fontSize)
      } else if (objectKeys.includes('fontFamily')) {
        attaCustomApiObject.setFontFamily(data.fontFamily)
      } else if (objectKeys.includes('color')) {
        attaCustomApiObject.setFontColor(data.color)
      } else if (objectKeys.includes('bold')) {
        attaCustomApiObject.setBold(data.bold)
      } else if (objectKeys.includes('italic')) {
        attaCustomApiObject.setItalic(data.italic)
      }
    }
    ...
    var mouseDown = function (evt) {
      var event = new CustomEvent('attaCustomMouseDown', {
        detail: {
          event: evt
        }
      });
      ...
      if (typeof (attatippyinstance) != 'undefined') {
        attatippyinstance.destroy();
        attatippywidgetopened = false;
      }
      if (!right_click) {
        //  let event = new CustomEvent("cutommousedownevent",{detail:{
        //   originalObject:opts,
        //   editorMode:mode,
        //   description:'cutommousedownevent from (ext-eyedropper.js line 85)'
        // }});
        // document.dispatchEvent(event);
        ...
        if (evt.target.tagName == "text") {
          ...
          var fontColor = evt.target.getAttribute("fill")
          var fontFamily = evt.target.getAttribute("font-family")
          var fontSize = evt.target.getAttribute("font-size");
          var fontWeight = evt.target.getAttribute("font-weight")
          var fontStyle = evt.target.getAttribute("font-style");
          ...
          attashowFloatingToolbar({
            type: 'text',
            node: evt.target,
            callback: attahandleTippyCommand,
            currentValues: {
      ...
      //atta code for selection updated
      var attaselectelement;
      if (selectedElements.length == 1 && (typeof (selectedElements[0]) != 'undefined')) {
        if((selectedElements[0]) != null){
          if (selectedElements[0].tagName == "g" || selectedElements[0].tagName == "text" || selectedElements[0].tagName == "path" || selectedElements[0].tagName == "image") {
            var tmpelementid=selectedElements[0].getAttribute('id');
            if(tmpelementid=="back"){
              tmpelementid ="background";
            }
            if(selectedElements[0].tagName == "g" && tmpelementid=="background" ){}
            else
            {
              attaselectelement = selectedElements[0]
            }
          }
        }
      }
      var attacutomselectionevent = new CustomEvent('attaSelectionUpdated', {
        detail: {
          selectedElement: attaselectelement
        }
      });
      document.dispatchEvent(attacutomselectionevent);
      //atta code ends
      ...
      switch (current_mode) {
        case "select":
          ...
          if (selectedElements[0] !== null) {
            ...
            if (dx != 0 || dy != 0) {
              if (typeof (attatippyinstance) != 'undefined' && selectedElements.length == 1 && selectedElements[0].tagName == "text") {
                attatippyinstance.destroy();
                attatippywidgetopened = false;
              }
              var len = selectedElements.length;
  ...
  attaCustomApiObject.setBold = this.setBold;
  ...
  attaCustomApiObject.setItalic = this.setItalic;
  ...
  attaCustomApiObject.setFontFamily = this.setFontFamily;
  ...
  attaCustomApiObject.setFontColor = this.setFontColor;
  ...
  attaCustomApiObject.setFontSize = this.setFontSize;
